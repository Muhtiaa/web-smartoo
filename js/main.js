...
function initTranslator() {
  ...
  if (match && match[1]) {
    currentLangText.textContent = match[1].toUpperCase();
  }
  ...
  langItems.forEach(item => {
    item.addEventListener("click", () => {
      ...
      // Set the Google Translate cookie manually for safety
      ...
      if (googleSelect) {
        ...
        // Dispatch event that bubbles so Google's listener catches it
        googleSelect.dispatchEvent(new Event("change", { bubbles: true }));
      } else {
        // Fallback: reload page so the cookie takes effect
